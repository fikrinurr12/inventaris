      ...
      <div class="card-body table-responsive">
        ...
        {{-- <table class="table text-center align-items-center mb-0 table-striped table-bordered" id="tabel_master_barang">
          ...
          <tbody>
              ...
              <tr>
                  <td class="text-sm">{{ $barang->kode }}</td>
                  <td class="text-sm">{{ $barang->nama }}</td>
                  <td class="text-sm">{{ $barang->merk }}</td>
                  <td class="text-sm">{{ $barang->kategori->nama ?? 'Tidak ada kategori' }}</td>
                  <td class="text-sm">
                    <button 
                        class="btn btn-outline-info btn-sm btn-lihat-keterangan square-btn d-flex align-items-center justify-content-center" 
                        data-foto="{{ $barang->foto }}"
                        data-keterangan="{{ $barang->keterangan }}" 
                        data-harga="{{ $barang->harga_terakhir }}"
                        data-spesifikasi="{{ $barang->spesifikasi }}"
                        data-stok_baik="{{ $barang->stok_total_baik }}"
                        data-stok_rusak="{{ $barang->stok_total_rusak }}"
                        data-stok_tersedia="{{ $barang->stok_tersedia }}">
                        <i class="fas fa-eye"></i>
                    </button>
                    </td>                
                  <td>
                      <a href="{{ route('edit_barang', $barang->id) }}" class="btn btn-sm btn-warning square-btn">
                          <i class="fas fa-pencil-alt"></i>
                      </a>
                      <form action="{{ route('hapus_barang', $barang->id) }}" method="POST" style="display: inline-block;">
                          @csrf
                          @method('DELETE')
...
<script>
$(document).ready( function () {
    var table = $('#tabel_master_barang').DataTable({
        processing: true,
        serverSide: true,
